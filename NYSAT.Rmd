...
```
```{r, Ehtesham Model part,include=FALSE}
head(df)
...
mahal = mahalanobis(MLR4,
                    colMeans(MLR4, na.rm = TRUE),
                    cov(MLR4, use = "pairwise.complete.obs"))
mahal
cutoff = qchisq(1-0.001,ncol(MLR4)) # detrming the cutoff point=20.515
print(cutoff)
summary(mahal < cutoff) # Determining the number of outliers. 12 outliers. WE are not taking out outliers due to two #reasons:
...
olsreg2Math8 = lm(`SAT Math Avg. Score` ~ MLR4)
summary(olsreg2Math8)
#8 Linearity check. 
